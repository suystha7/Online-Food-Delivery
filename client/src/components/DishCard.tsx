...
const getStarRating = (rating: number) => {
  ...
  return (
    <>
      {Array(fullStars)
        .fill(<FaStar size={20} key="full" className="text-yellow-400" />)
        .map((el, i) => (
          <span key={`full-${i}`}>{el}</span>
        ))}
      {halfStar && (
        <FaStarHalfAlt size={20} key="half" className="text-yellow-400" />
      )}
      {Array(emptyStars)
        .fill(<FaRegStar size={20} key="empty" className="text-yellow-400" />)
        .map((el, i) => (
