...
const Header: React.FC = () => {
  ...
  const dropDownItemStyle =
    "flex gap-2 items-center px-4 py-2 text-gray-700 hover:bg-gray-100 cursor-pointer text-base border-b border-gray-200 last:border-b-0";

  return (
    <header
      className="w-full fixed top-0 left-0 z-[100] py-4 pr-4 md:pr-12 duration-300"
      ref={headerRef}
    >
      <div className="flex items-center justify-between px-4 md:px-16">
        ...
        {/* Logo */}
        <div className="logo flex justify-center md:justify-center w-full md:w-auto">
          <Link href="/">
            <Image
              src="/logo.png"
              width={100}
        ...
        {/* Navigation */}
        <nav
          className={`absolute top-full left-0 bg-white w-full shadow-md transform transition-all duration-300 ${
            isMobileMenuOpen ? "translate-x-0" : "-translate-x-full"
          } md:static md:top-auto md:left-auto md:bg-transparent md:w-auto md:shadow-none md:transform-none md:flex md:gap-8 md:items-center`}
        >
            ...
            {NAV_ITEM_LIST.map((navItem) => (
              <li key={navItem.id}>
                <Link
                  ...
                  className={`$\{
                    pathName === navItem.navigateTo ? "text-white" : "text-gray-700"
